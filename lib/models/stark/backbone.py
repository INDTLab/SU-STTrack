...
class BackboneBase(nn.Module):
    ...
    def __init__(self, backbone: nn.Module, train_backbone: bool, num_channels: int, return_interm_layers: bool,
        ...
        if return_interm_layers:
            ...
            if net_type == "resnet":
                low_layers = {"layer2": "0"}  #layer2:[16, 512, 16, 16]
        ...
        self.low_layers = IntermediateLayerGetter(backbone, return_layers=low_layers)
        self.num_channels = num_channels

    def forward(self, tensor_list: NestedTensor):
        xs = self.body(tensor_list.tensors)
        low_feat = self.low_layers(tensor_list.tensors)
...
class Joiner(nn.Sequential):
    ...
    def forward(self, tensor_list: NestedTensor, mode=None):
        xs,low_feat = self[0](tensor_list)
