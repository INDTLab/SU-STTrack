...
class UVOTDataset(BaseDataset):
    """
    UVOT test set
    """
    def __init__(self):
        super().__init__()
        self.base_path = os.path.join(self.env_settings.uvot_path, 'test')
    ...
    def _construct_sequence(self, sequence_name):
        ...
        if self.dir_type == 'one-level':
            return Sequence(sequence_name, frames_list, 'uvot', ground_truth_rect.reshape(-1, 4))
        elif self.dir_type == 'two-level':
            return Sequence(seq_name, frames_list, 'uvot', ground_truth_rect.reshape(-1, 4))
